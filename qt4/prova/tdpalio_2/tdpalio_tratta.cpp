//
// C++ Implementation: tdpalio_tratta
//
// Description:
//
//
...
void TDPalio::SceltaCavalli() {
    eventLabel->setText("I capitani si riuniscono\nper la scelta dei cavalli...");
    ...
    //for(int i=0; i<21; i++)
    //  cout << cav[i] << "  " << cav1[i] << " " << media[i] << endl;
    for(int i=0; i<21; i++) {
        for(int j=0; j<21; j++) {
            if (cav[i] == cav1[j]) {
                if (j < 7)
    ...
    while(1) {
        ...
        for(int c=0; c<10; ++c) {
            ...
            if (contrade[corrono[c]].giocatore() != -1) {
                ...
                Tratta t(this, "tratta");
                t.cambiaCaption(contrade[corrono[c]].getNomeCompleto());
                t.riempiLista(nomi, corsi, vinti, 10 - totale);
                t.exec();

                selezione_giocatore = t.getSelezione();
                for(int j=0; j<21; j++)
                    if (selezione_giocatore[j])
                        voti[j]++;
            } else {
                int strategia[3];
...
void TDPalio::Assegnazione(int* cav) {
    eventLabel->setText("Assegnazione");
...
void TDPalio::SceltaStrategia() {
    ...
    for(int i=0; i<10; i++) {
        ...
        barberi[i].setStrategia(strategia.value(media1, media2,
