...
class Polygon: public QCanvasRectangle 
{
public:
  Polygon(int, int, int, int, QCanvas*);
...
Polygon::Polygon(int x, int y, int dx, int dy, QCanvas* q)
  :QCanvasRectangle(x, y, dx, dy, q) {

  std::cout << "PIPPO1" << std::endl;
  text = new QCanvasText(q);
  //text->setText("QCanvasText");
  //text->move(10, 10);
  //text->setZ(1);
  //text->show(); 
}

QPointArray Polygon::areaPoints() const {
  return areaPoints();
}
...
void Main::addButterfly()
{
    if (butterfly_fn.isEmpty())
        return;
    if (!butterflyimg) {
        butterflyimg = new QImage;
        butterflyimg->load(butterfly_fn);
        ...
        *butterflyimg = butterflyimg->smoothScale(int(butterflyimg->width()*0.15), int(butterflyimg->height()*0.15) );
    }
    QCanvasPolygonalItem* i = new ImageItem(*butterflyimg, &canvas);
...
void Main::addLogo() {
  ...
  if (!logoimg) {
    ...
    float w = (float)canvas.width()/(float)logoimg->width();
    std::cout << canvas.width() << std::endl;
    std::cout << int((float)logoimg->width()*w) << std::endl;
    //logoimg->smoothScale(int((float)logoimg->width()*w),
    //			 int((float)logoimg->height()*w));
    *logoimg = logoimg->scale(canvas.width(), canvas.height());
    std::cout << logoimg->width() << std::endl;
  }
  QCanvasPolygonalItem* i = new ImageItem(*logoimg, &canvas);
  //i->move((double)(canvas.width()-logoimg->width()),
  //	  (double)(canvas.height()-logoimg->width()));
  i->setZ(256);
  i->show();
}
...
void Main::addRectangle() {
  ...
  */
  QCanvasPolygonalItem* i = new Polygon(rand()%canvas.width(),rand()%canvas.height(), canvas.width()/5,canvas.width()/5,&canvas);
  std::cout << i << std::endl;
  //int z = rand()%256;
  std::cout << "a" << std::endl;
  i->setBrush(QColor(10,10,10));
  std::cout << "b" << std::endl;
  i->setPen( QPen(QColor(25, 25, 25), 6) );
  std::cout << "c" << std::endl;
  i->setZ(10);
  i->show();  
  std::cout << "c" << std::endl;
