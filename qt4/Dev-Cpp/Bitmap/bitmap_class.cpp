...
bool DIB_BITMAP::set_size(HDC hdc, int width, int height)	
{
...
bool DIB_BITMAP::set_palette(int start_color, int num_colors, RGBQUAD *colors)
{	
...
bool DIB_BITMAP::loadBMP(char *file_name, HDC hdc)
{
	...
	// Now we're creating an "empty" DIB_BITMAP for storing the 8-bit bitmap
	...
	if(!set_size(hdc,bmp_infoheader.biWidth, bmp_infoheader.biHeight))
		goto CLEAN_UP;
...
/************* HERE WE'RE GOING TO READ THE COLOR MAP INFORMATION ***************/
	...
	if(!set_palette(0,tableSize,rgbQuad))
		goto CLEAN_UP;
...
/*************** DONE READING THE COLOR MAP INFORMATION *************************/
	...
	else if(bmp_infoheader.biCompression == BI_RGB) // There is NO compression
	{
		...
		*/
		...
		// and store what we read in, in our DIB_BITMAP :)
		for(int y = 0; y < bmp_infoheader.biHeight; y++)
		{
			// We have to flip the y-coordinate because our DIB_BITMAP
