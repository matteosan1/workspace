...
void PSheet::OnChangePrice()
{
    ...
    float prezzo_temp = Rounded((prezzo_netto-sconto-spedizione_posta+spedizione)/1.2);
...
void PSheet::PrintAddress()
{
    ...
    char b[5];
    SelectObject(pd.hDC, hfontStd1);
    TextOut(pd.hDC, 62*wChar, yChar * 11 + iLine, "Gent.:", 6);
    ...
    TextOut(pd.hDC, 37*wChar, yChar * 18 + iLine, data.c_str(), data.length());
}
...
void PSheet::PrintTable(bool h)
{
    ...
    for(int i=0; i<cd_collo.size(); i++)
    {
        ...
        if ((int)r1[10] != 2)
            prezzo_temp = (float)r1[8];
        else
            prezzo_temp = (float)r1[9];
