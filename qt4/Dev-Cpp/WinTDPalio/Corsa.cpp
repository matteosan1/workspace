...
void Corsa::Batterie(Cavallo* c, HWND hwnd)
{
    int n[20];
    Ordina(n, 20);
    ...
    ListDlg pippo(hwnd, lista.c_str());  
...
void Corsa::Estrazione(Gioco* g, HWND hwnd)
{
    int* n;
    if (g->IsLuglio())
        n = g->GetLuglio();
    ...
    for(int i=0; i<7; i++)
        theDiritto[i] = g->GetContrada(n[i+10]);
       
    string lista;    
    for(int i=0; i<10; i++)
    {
        lista += theCorrenti[i].GetNome();
        lista += "\n";
    }   
